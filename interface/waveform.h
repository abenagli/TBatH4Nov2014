...
public:
  ...
  TGraph* getGraph()
  {
    ...
    {
      graph -> SetPoint(i,_times.at(i),_samples.at(i));
    }
    return graph;
  }
  
  // get DFT of the waveform
  std::vector<complessi> getDFT() { return DFT<double>(_samples); }
  
  TGraph* getDFTPowerSpectrumGraph()
  {
    std::vector<complessi> dft = DFT<double>(_samples);
    
    TGraph* graph = new TGraph();
    for(unsigned int i(0); i < _samples.size(); ++i)
    {
      graph -> SetPoint(i,i,(dft.at(i)).Mod());
    }
    return graph;
  }
  
  void setIDFTWaveform()
  {
    std::vector<complessi> dft = DFT<double>(_samples);
    for(unsigned int i(0); i < dft.size(); ++i)
    {
      if( (i > 50 && i < 70) || (i > 930 && i < 950) )
        dft.at(i) = 0.;
    }
    std::vector<complessi> idft = IDFT<complessi>(dft);
    for(unsigned int i(0); i < _samples.size(); ++i)
    {
      _samples.at(i) = (idft.at(i)).Re();
    }
  }
  ...
  //Get the trigger time at a given fraction of the amplitude
  float trg_time_at_frac(const int& x1, const int& x2, const float& frac, int SampleToInterpolate=5) const;

  //Get the trigger time at a given fraction of the amplitude
  float trg_time_at_frac(const float& t1, const float& t2, const float& frac, int SampleToInterpolate=5) const;
