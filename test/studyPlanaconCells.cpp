...
{  
  std::cout<<"--------> DUMPER: READ RECO DATA AND PRODUCE PLANACON CELL PLOTS <--------"<<std::endl;
  ...
  //---------output histograms----------------
  TFile* outputFile = TFile::Open(Form("plots/plots_studyPlanaconCells_%s.root",outputLabel.c_str()),"RECREATE");
  ...
  std::map<int,TH2F*> h2_cellPosition_TDC;
  std::map<int,TH2F*> h2_cellPosition_hodo12;
  ...
  {
    h2_cellPosition_TDC[iCh]    = new TH2F(Form("h2_cell%02dPosition_TDC",iCh),   "",100,-240.,-140.,100,-146.,-46.);
    h2_cellPosition_hodo12[iCh] = new TH2F(Form("h2_cell%02dPosition_hodo12",iCh),"",160,-210.,-130.,160,-155.,-75.);
  ...
  for(int iEntry = 0; iEntry < entriesMax; ++iEntry)
  {
    ...
    for(int iCh = 0; iCh < 32; ++iCh)
    {
      if( recoTV.amp_DQM[iCh] > 500 ) h2_cellPosition_TDC[iCh]    -> Fill(beamX_TDC,beamY_TDC);
      if( recoTV.amp_DQM[iCh] > 500 ) h2_cellPosition_hodo12[iCh] -> Fill(beamX_hodo1,beamY_hodo2);
