...
Waveform::max_amplitude_informations Waveform::max_amplitude(const int& x1, const int& x2, int nSamplesAroundMax) const
{
  ...
  if( imax > -1 )
  {
    ...
    for(int i(0); i < nSamplesAroundMax; ++i)
    {
      ...
      {
        x[i]=(imax-(nSamplesAroundMax-1)/2+i)/FSAMPLE;
    ...
    {
      if( WARNING_ERROR )
        std::cout << "WARNING::Waveform::max_amplitude::not enough samples to fit for maximum. Returning unfitted position" << std::endl;
...
//Get the time at a given fraction of the amplitude for times between x1 and x2 
float Waveform::trg_time_at_frac(const float& t1, const float& t2, const float& frac, int SampleToInterpolate) const
{
  int x1 = 0;
  int x2 = 0;
  for (unsigned int i(0);i<_times.size();++i)
  ...
  return trg_time_at_frac(x1,x2,frac,SampleToInterpolate);
}



//Get the time at a given fraction of the amplitude for times between x1 and x2 
float Waveform::trg_time_at_frac(const int& x1, const int& x2, const float& frac, int SampleToInterpolate) const
{
  double signal = 0.;
  double baseline = 0.;
  int nSamplesSignal = 0;
  int nSamplesBaseline = 0;
  
  for(unsigned int i = 0; i < _samples.size(); ++i)
  {
    if( int(i) < x1 ) { signal   += _samples.at(i); ++nSamplesSignal; };
    if( int(i) > x2 ) { baseline += _samples.at(i); ++nSamplesBaseline; };
  }
  
  signal /= nSamplesSignal;
  baseline /=  nSamplesBaseline;
  double amplitude = signal - baseline;
  double threshold = baseline + frac*amplitude;
  
  int cfSample = 0;
  for(unsigned int i = 0; i < _samples.size(); ++i)
  {
    if( _samples.at(i) < threshold )
    {
      cfSample = i;
      break;
    }
  ...
  if( cfSample > -1 )
  {
    ...
    for(int i(0); i < SampleToInterpolate; ++i)
    {
      if( (cfSample-(SampleToInterpolate-1)/2+i) >= max(x1,0) && (cfSample-(SampleToInterpolate-1)/2+i) <= min(x2,(int)_samples.size()) )
      {
        x[i] =   _times[cfSample-(SampleToInterpolate-1)/2+i];
        y[i] = _samples[cfSample-(SampleToInterpolate-1)/2+i];
        syy += y[i] * y[i];
        sxy += x[i] * y[i];
        sxx += x[i] * x[i];
      ...
      {
        if( WARNING_ERROR )
          std::cout << "WARNING::Waveform::trigger threshold found too close to gate edges. Increase gate width" << std::endl;
      }
    }
    
    if( nSamples > 1 )
    {
      //Now fit with a linear function y = a+bx around threshold value
      double b = ( nSamples*sxy - sx*sy) / ( nSamples*sxx - sx*sx );
      double a = ( sy - b*sx) / (1.*nSamples);
      // TF1* fitFunc = new TF1("fitFunc","[0]+[1]*x",140.,180.);
      // fitFunc -> SetParameters(a,b);
      // fitFunc->Draw("same");
...
std::vector<float> Waveform::time_at_threshold(const int& x1, const int& x2, const float& threshold, int SampleToInterpolate) const 
{
  ...
  for(unsigned int icross = 0; icross < cfSample.size(); ++icross)
  {
    double x[SampleToInterpolate];
    double y[SampleToInterpolate];
    double syy=0,sxy=0,sxx=0,sx=0,sy=0;
