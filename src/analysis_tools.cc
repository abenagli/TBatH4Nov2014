...
bool FillChain(TChain* chain, const std::string& inputFileList)
{
  ...
  if( !inFile.is_open() )
  {
    std::cerr << "** ERROR: Can't open '" << inputFileList << "' for input file list" << std::endl;
    return false;
  }
  while(1)
  {
    ...
    std::cout << ">>> analysis_tools::FillChain - treeName = " << chain->GetName() << " from file " << buffer << std::endl;
  }
  
  std::cout << ">>> analysis_tools::FillChain - read " << chain->GetEntries() << " entries" << std::endl;
  return true;
}
...
int GetPlanaconId(const int& digiId, const unsigned int& runNumber)
{
  ...
  {
    if( digiId != 6 && digiId != 7 )
    {
      return digiId;
    }
    else
    {
      if( digiId == 6 ) return 7;
      else              return -1;
    }
  }
}
...
float GetHodoWire(const int& nHits, int* hits)
{
  ...
  //   std::cout << "ii: " << ii << "   val: " << hits[ii] << std::endl;
  // }
  ...
  {
    std::vector<std::pair<int,int> > consecutives;
    for(int ii = 0; ii < nHits-1; ++ii)
    {
      if( abs(hits[ii+1]-hits[ii]) == 1 )
      {
        consecutives.push_back( std::make_pair<int,int>(hits[ii],hits[ii+1]) );
        //std::cout << "consecutive " << hits[ii] << "-" << hits[ii+1] << std::endl;
      }
    }
    
    if( consecutives.size() == 1 )
      return 0.5 * ( consecutives.at(0).first + consecutives.at(0).second );
    else if( (consecutives.size() == 2) && (consecutives.at(1).first == consecutives.at(0).second) )
      return 1. * consecutives.at(0).second;
    else return -1.;
  }
}
...
std::pair<double,double> CalculatePedestal(std::vector<float>* v, const int& firstSample, const int& lastSample)
{
  double val = 0.;
  double val2 = 0.;
  for(int it = firstSample; it < std::min(lastSample,int(v->size())); ++it)
  {
    val += v->at(it);
    val2 += (v->at(it))*(v->at(it));
  }
  
  int N = lastSample-firstSample;
  double ped = 1.*val/N;
  double noise = sqrt( 1./(N-1) * (val2-N*ped*ped) );
  
  return std::make_pair<double,double>(ped,noise);
}



void AddWaveform(TGraph* g, std::vector<float>* v, const bool& pedSub, const int& firstSample, const int& lastSample)
{
  double ped = 0.;
  if( pedSub ) ped = (CalculatePedestal(v,firstSample,lastSample)).first;
